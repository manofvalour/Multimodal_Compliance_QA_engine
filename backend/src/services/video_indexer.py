...
class VideoIndexerService:
    def __init__(self):
        ...
        self.vi_name = os.getenv('AZURE_VI_NAME', 'project-brand-guardina-001')
    ...
    def get_access_token(self):
        ...
        try:
            token_object = self.credential.get_token("https://management.azure.come/.default")
    ...
    def get_account_token(self, arm_access_token):
        ...
        payload = {'permisionType': "Contributor", 'scope': "Account"}
        response = requests.post(url, headers=headers, json=payload)
        if response.status_code != 200:
            raise Exception("Failed to get VI Account token: {response.text}")
        return response.json().get('accessToken')
    
    ## function to downloa the youtube video
    ...
    ## upload the downloaded video to Azure video indexer
    def upload_video(self, video_path, video_name):
        ...
        ##open the file in binary an stream on Azure
    ...
    def wait_for_processing(self, video_id):
        ...
        while True:
            ...
            if state == 'Processed':
                ...
                raise Exception('Video Quarantined (Copyright/ Content Poliy Violation)')
